...
public class RemoveDuplicate {
  ...
  private static int removeDuplicates(int[] nums) {
    ...
    System.out.println("Before removing the elements : " + Arrays.toString(nums));
    int writeElement = 1;
    for (int readElemet = 1; readElemet < nums.length; readElemet++) {
      if (nums[readElemet] != nums[readElemet - 1]) {
        nums[writeElement] = readElemet;
        writeElement++;
      }
    }
    System.out.println("After Removing the element : " + Arrays.toString(nums));
    return writeElement;
  }
